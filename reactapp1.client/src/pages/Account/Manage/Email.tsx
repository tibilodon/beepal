...
function Email() {
  ...
  return (
    <>
      ...
      {notification && (
        <p className="success">
          Megerősítő e-mail elküldve. Kérlek ellenőrizd az új e-mail címed!
        </p>
      )}

      {validationErrors.Error && (
        <span className="text-danger">{validationErrors.Error[0]}</span>
      )}
        ...
        <form method="post" onSubmit={submitHandler}>
          <div className={styles.inputs}>
            <span className={styles.inputWrap}>
              <h6>Jelenlegi e-mail cím:</h6>
              <span className={styles.flexWrap}>
                <p>{formData.email}</p>
                <p className={styles.verfified}>✓</p>
              </span>
            </span>
          ...
          <ButtonA
            label="Mentés"
            disabled={!formData.newEmail && true}
            type="submit"
          />
        </form>
      </div>
